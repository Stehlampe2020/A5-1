...
#  cipher.py
#
#  Author: Stehlampe2020
#  Based on the work by: overxfl0w13
#
...
def stepFive(lfsr1, lfsr2, lfsr3):
    keyStream = ""
    keyStream += str(
        lfsrOne._getBit(lfsrOne._getLength() - 1) ^ lfsrTwo._getBit(lfsrTwo._getLength() - 1) ^ lfsrThree._getBit(22))
    for i in range(227):
        clockingBits = [lfsrOne._getClockingBit(), lfsrTwo._getClockingBit(), lfsrThree._getClockingBit()]
        oneCount, zeroCount = clockingBits.count(1), clockingBits.count(0)
        majorityBit = 1 if max(oneCount, zeroCount) == oneCount else 0
        ...
        if lfsrThree._getClockingBit() == majorityBit:
            ...
            lfsrThree._setRegister([nMsb] + lfsrThree._getRegister()[0:lfsrThree._getLength() - 1])
        keyStream += str(
            lfsrOne._getBit(lfsrOne._getLength() - 1) ^ lfsrTwo._getBit(lfsrTwo._getLength() - 1) ^ lfsrThree._getBit(
                22))
    return keyStream


def stepSix(plainText, keyStream): return "".join(
    [str(xor(keyStream[i % 228], plainText[i])) for i in range(len(plainText))])
